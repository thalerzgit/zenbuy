...
- **A new session always opens on the mode picker.** Cold launch (and **Restart** on a finished report) lands on "What do you want to do?" — **Find stocks** or **Analyze tickers you know** — even if a previous session finished the wizard. Style (Aggressive Growth, …) and profit window (0–3 yrs, …) stay stored as defaults for those later steps; they must not appear on first open. "Change setup" on the unlocked screen reopens the wizard from step 1 without wiping an in-progress session. The Siri Remote's Menu button still pops a pushed screen — that is the system affordance, and the app draws no button for it.
- **Chrome is the Dist icon, no “Apple TV” badge.** `TVContentView` draws one forest header on every launched screen (home, wizard, report). It uses `BrandIcon` (the same ZB + candles art as the home-screen App Icon) in the top-left with a ZenBuy wordmark. There is no platform label in the top-right.
...
Email PDF opens an inline card next to the bar that was clicked — inline rather than an alert because focusing a tvOS text field is what raises the system keyboard. The field is a UIKit `UITextField` (`TVEmailTextField`) with `keyboardType = .emailAddress` and **no** `textContentType`. Setting `.emailAddress` content type is what summons the system Previously-Used Emails sheet; that sheet plus `becomeFirstResponder` on SwiftUI focus restore looped after Justin picked `gary.morgenthaler@iCloud.com`. Send PDF still reads the coordinator's `currentText` (updated on every `.editingChanged`), not a SwiftUI `TextField` binding. Opening the panel — or finishing the field — parks the remote on **Send PDF** when an address is already there, so the gold ring does not sit on Email PDF and re-open the sheet. Empty-field, bad-format, and server errors use distinct copy. The address is remembered in `@AppStorage` so a second send is one click. Closing the card hands focus back to the share button it came from.
